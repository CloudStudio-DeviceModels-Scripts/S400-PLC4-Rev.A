    ...
    switch (tag){
        case "/PLC4/ALM_H_PIT01":
            ...
            tagvalues.forEach(valueElement => {
                ...
                var etv1 = device.endpoints.byAddress("ALM_H_PIT01");
                etv1.updateApplianceStatus(ValueData.v, ValueData.ts);  
            });
            break;

        case "/PLC4/ALM_Blowers":
            //Obtenemos el array con lecturas de este tag
            var tagvalues = N3uronData[tag];
            //Recorremos las lecturas y leemos los valores v, q y ts
            tagvalues.forEach(valueElement => {
                var ValueData = ExtractTagData(valueElement);
                //Listo, actualizamos el endpoint
                var etv1 = device.endpoints.byAddress("ALM_Blowers");
                etv1.updateApplianceStatus(ValueData.v, ValueData.ts);
            });
            break;
            ...
            tagvalues.forEach(valueElement => {
                var ValueData = ExtractTagData(valueElement);
                //Listo, actualizamos el endpoint
                var etv1 = device.endpoints.byAddress("40-PIT-01");
